...
describe("todoRepository", () => {
  ...
  describe("getByID", () => {
    it("should result todo", async () => {
      const connection = await createDBConnection();
      const todos = await createTodoTestDatas(connection, 1);
      const todo = todos[0];

      const repoository = new TodoRepository(connection);
      const getByIdResult = await repoository.getByID(todo.id!);
  ...
  describe("create", () => {
    it("should return createdID", async () => {
      ...
      const selectResult = await getTodoByIdTest(connection, createdID);
  ...
  describe("update", () => {
    it("should update todo and return updatedID", async () => {
      ...
      expect(updateTodo.id).toBe(result.id);
  ...
  describe("delete", () => {
    it("should delete todo and return deleteID", async () => {
      ...
      expect(selectResult).toBe(undefined);
  ...
  async function getTodoByIdTest(connection: Connection, id: number): Promise<Todo> {
